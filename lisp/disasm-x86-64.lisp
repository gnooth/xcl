...
(defun error-unhandled-byte-sequence (byte1 byte2)
  ...
  (let* ((bytes (designator-list byte-or-bytes))
    ...
    `(progn
       (defun ,name ,args
         ...
         (mumble "deprecated handler ~S called~%" ',name)
         ,@body)
       (dolist (byte ',bytes)
...
(defmacro define-disassembler (byte-or-bytes &body body)
  (let* ((bytes (designator-list byte-or-bytes))
    ...
    `(progn
       (defun ,name ,args
         (declare (ignorable ,@args))
;;          (mumble "disassembler ~S called~%" ',name)
...
(define-handler #x6b
  (let ((byte2 (mref-8 start (1+ offset))))
    (cond ((and (eql prefix-byte #x48)
          ...
          (t
           (error-unhandled-byte-sequence #x6b byte2)))))

(define-disassembler (#x70 #x71 #x72 #x73 #x74 #x75 #x76 #x77
                      #x78 #x79 #x7a #x7b #x7c #x7d #x7e #x7f)
  (mumble "short jmp disassembler byte1 = #x~2,'0x~%" byte1)
...
;;                                 (setq instruction (process-jcc (+ block-start offset)))
                                (let* ((displacement (mref-32 block-start (+ offset 2)))
                                       (absolute-address (ldb (byte 32 0) (+ block-start offset 6 displacement))))
                                  (setq length 6
                                        mnemonic (case byte2
                                                   (#x80 :jo)
                                                   (#x81 :jno)
