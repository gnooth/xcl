...
(defun end-block (xp suffix)
  (unless (eq *abbreviation-happened* '*print-lines*)
	...
	((not (< ptr (Qright xp))))    ;all but last
      (when (and (eql (depth-in-blocks xp) (Qdepth xp ptr))
...
(defun flush (xp)
  (unless *locating-circularities*
    (sys:%stream-write-string (base-stream xp) (buffer xp) 0 (buffer-ptr xp)))
  ...
  (let* ((out-point (BP<-TP xp (Qpos xp Qentry)))
    ...
    (unless *locating-circularities*
      (let ((stream (base-stream xp)))
        (sys:%stream-write-string stream (buffer xp) 0 end)
...
(defun set-suffix (xp suffix-string)
  (let* ((end (length suffix-string))
    ...
    (do ((i (1- new-end) (1- i)) (j 0 (1+ j))) ((eql j end))
...
(defun basic-write (object stream)
  (cond ((xp-structure-p stream)
         (write+ object stream))
	(*print-pretty*
         (maybe-initiate-xp-printing #'(lambda (s o) (write+ o s)) stream object))
	(t
...
;; 		(write-char++ #\" xp) T))
         ...
         t)
	((typep object 'fixnum)
	 (when (and (null *print-radix*) (eql *print-base* 10.))
	   (when (minusp object)
	     (write-char++ #\- xp)
	     (setq object (- object)))
...
;; 		  (if mode (pop-char-mode xp)) T))))
        ((and (symbolp object)
         ...
         (let* ((*print-pretty* nil)
           ...
           (write-string++ string xp 0 (length string))
           t))))
...
;; ;;     (if (xp-structure-p stream)
;; ;;         (basic-write object stream)
;; ;;         (sys:%stream-write-object stream object)))
;;   (let ((*print-escape* nil))
;;     (cond ((xp-structure-p stream)
;;            (write+ object stream))
;;           (t
;;            (setq stream (sys:designator-output-stream stream))
;;            (if *print-pretty*
;;                (maybe-initiate-xp-printing #'(lambda (s o) (write+ o s)) stream object)
;;                (sys:%stream-write-object stream object)))))
;;   object)
...
(defun write-char (char &optional (stream *standard-output*))
  ...
  (if (xp-structure-p stream)
      ...
      (sys:%stream-write-char stream char))
  char)
...
(defun write-string (string &optional stream &key (start 0) end)
  ...
  (if (xp-structure-p stream)
      ...
      (sys:%stream-write-string stream string start end))
  string)
...
(defun params-end (start) ;start points just after ~
  (let ((j start) (end (length *string*)))
    (loop
      ...
      (incf j)
      (if (eql j end) (err 2 "No character after '" (1- j)))
      (incf j))))
...
;;          (write-string (sys:%write-to-string (aref array)) xp)
         (write+ (aref array) xp)
         )
	(t
...
(defun output-pretty-object (object stream)
  (if (xp::xp-structure-p stream)
      ...
      (xp::maybe-initiate-xp-printing #'(lambda (s o) (xp::write+ o s)) stream object)))
