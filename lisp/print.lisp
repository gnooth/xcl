...
(defun output-integer (integer stream)
  (if (xp::xp-structure-p stream)
      (let ((s (%write-to-string integer)))
        (xp::write-string++ s stream 0 (length s)))
      (%stream-write-object stream integer)))
...
(defun output-vector (vector stream)
  ...
  (cond ((stringp vector)
         (assert nil)
         ;; FIXME not reached
         (%stream-write-object stream vector))
...
;;                (%stream-write-object stream object)))))
    (unless (xp::xp-structure-p stream)
      (setq stream (designator-output-stream stream)))
...
(defun write (object &key
  ...
  ;; REVIEW
  (when (and pretty (autoloadp 'pprint))
    (resolve 'pprint)
    (when (null pprint-dispatch)
      (setq pprint-dispatch *print-pprint-dispatch*)))
