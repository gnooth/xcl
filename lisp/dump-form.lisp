...
(defun dump-vector (vector stream)
  ...
  (let ((length (length vector)))
    (cond ((equal (array-element-type vector) '(unsigned-byte 8))
;;            (mumble "dump-vector ub-8 case~%")
          ...
          ((and (eql length 3)
                (eq (aref vector 0) :bytes))
;;            (mumble "dump-vector new case~%")
;;            (%stream-write-char stream #\#)
;;            (%stream-write-object stream length)
;;            (%stream-write-char stream #\()
           (let ((data-length (aref vector 1))
                 (data (aref vector 2)))
             (aver (fixnump data-length))
             (aver (listp data))
             (aver (length-eql data data-length))
;;              (dump-object :BYTE-VECTOR stream)
;;              (%stream-write-char stream #\space)
;;              (dump-object data-length stream)
;;              (%stream-write-char stream #\space)
             (%stream-write-char stream #\#)
             (%stream-write-object stream data-length)
             (%stream-write-char stream #\%)
             (dolist (byte data)
               (sys:%stream-write-char stream (code-char byte)))
;;              (%stream-write-char stream #\))
             ))
          ((> length 0)
           (%stream-write-char stream #\#)
