  ...
  (when (check-arg-count test-form 2)
    (let* ((args (%cdr test-form))
      ...
      (when (or (characterp arg1) (fixnump arg1))
        (mumble "%p2-test-eq swapping args~%")
        (let ((temp arg1))
          (setq arg1 arg2
                arg2 temp))
...
(defun p2-cdr (form target)
;;   (when (zerop *safety*)
;;     (mumble "p2-cdr zero safety case target = ~S~%" target)
            ...
            (dolist (var (compiland-arg-vars compiland))
              ...
              (when (var-used-non-locally-p var)
                ...
                (when (var-arg-register var)
                  (aver (null (var-index var)))
                  (emit-move-register-to-relative (var-arg-register var) :rdi (var-closure-index var))
          ...
          (when (dolist (var (compiland-arg-vars compiland))
            ...
            (dolist (var (compiland-arg-vars compiland))
              (when (var-used-non-locally-p var)
                ...
                (when (var-arg-register var)
                  (aver (null (var-index var)))
                  (emit-move-register-to-relative (var-arg-register var) :rax (var-closure-index var))
                  (setf (var-arg-register var) nil)))))))
