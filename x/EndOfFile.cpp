...
bool EndOfFile::typep(Value type) const
{
  if (symbolp(type))
    return (type == S_end_of_file || type == S_stream_error || type == S_error
            || type == S_serious_condition || type == S_condition
            || type == S_standard_object || type == S_atom || type == T);
  else
    return (type == C_end_of_file || type == C_stream_error || type == C_error
