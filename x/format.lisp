...
(defpackage "FORMAT" (:use "CL" "SYSTEM" "EXTENSIONS"))
...
(defun round-up (string)
  ...
  (let* ((index (position #\. string))
         (n (read-from-string (setq string (remove #\. string))))
         (s (princ-to-string (incf n))))
    (loop for char across string
      while (eql char #\0)
      do (setq s (concatenate 'string "0" s)))
    (cond ((null index)
           s)
          (t
               ...
               (cond ((< actual-fdigits fdigits)
                     ...
                     ((> actual-fdigits fdigits)
                      (let* ((desired-length (+ index 1 fdigits))
                             (c (char s desired-length)))
                        (setq s (subseq s 0 (+ index 1 fdigits))
                              length (length s)
                              index (position #\. s))
                        (when (char>= c #\5)
                          (setq s (round-up s)
             ...
             (let ((minimum-width (+ (1+ index) (or fdigits 0))))
               ...
               (when (> length minimum-width)
                 ...
                 (when (every #'digit-char-p (subseq s (1+ index)))
                   (let ((c (char s minimum-width)))
                     (setq s (subseq s 0 minimum-width)
...
(defun nstring-capitalize-first (string)
...
(defun parse-conditional-directive (directives)
  (let ((sublists nil)
	...
	(let ((posn (position close-or-semi remaining)))
	  ...
	  (setq remaining (nthcdr (1+ posn) remaining))
	  (when (char= (format-directive-character close-or-semi) #\])
	    (return))
	  (setq last-semi-with-colon-p
	    ...
	    (multiple-value-bind (false-guts false-args false-simple)
              (let ((*simple-args* *simple-args*)
                ...
                (values (expand-directive-list false)
                        *simple-args*
                        *only-simple-args*))
	      (if (eql (length true-args) (length false-args))
		  `(if ,arg
		       (progn
			 ,@true-guts)
        ...
        (block nil
          ,@(when newline-segment-p
              `((setf newline-segment
                ...
                ,(expand-bind-defaults
                  ((extra 0)
                   (line-len '(or #+sbcl(sb!impl::line-length stream) 72)))
...
(defun extract-user-fun-name (string start end)
  (let ((slash (position #\/ string :start start :end (1- end)
	...
	(error 'format-error
      ...
      (intern (cond
               ((and second-colon (eql second-colon (1+ first-colon)))
...
(defun format-print-ordinal (stream n)
  ...
  (let ((number (abs n)))
	...
	(cond ((eql bot 12) (write-string "twelfth" stream))
	      ((eql tens 1)
...
(defun format-fixed-aux (stream number w d k ovf pad atsign)
  (cond
   ;; FIXME
;;    ((and (floatp number)
;;          #-xcl (or (sys:float-infinity-p number)
  ...
  (let ((close (find-directive directives #\) nil)))
    ...
    (interpret-bind-defaults () params
                               (let* ((posn (position close directives))
                                      ...
                                      (string nil))
                                 (let ((stream (make-string-output-stream)))
                                   (setq args (interpret-directive-list stream before orig-args args))
                                   (setq string (get-output-stream-string stream)))
                                 (write-string (if colonp
                                                   (if atsignp
                                                   ...
                                                   (if atsignp
                                                       (nstring-capitalize-first string)
     ...
     (setf args
           (catch 'up-and-out
             (when (and first-semi (format-directive-colonp first-semi))
               (interpret-bind-defaults
                ((extra 0)
                 (len (or #+sbcl(sb!impl::line-length stream) 72)))
                (format-directive-params first-semi)
                (setf newline-string
                      (with-output-to-string (stream)
                        (setf args
