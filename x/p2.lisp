...
;;   (push thing *code*)
  (ecase *current-segment*
    (:main
     (push thing *main*)
;;      (format t "emit *main* = ~S~%" *main*)
     )
    (:elsewhere
     (push thing *elsewhere*)
;;      (format t "emit *elsewhere* = ~S~%" *elsewhere*)
     ))
  )
  ...
  (when (length-eql form 3)
    (let* ((args (%cdr form))
      ...
      (when (and (neq type1 :unknown)
        ...
        (format t "p2-char derived type is subtypep simple-string~%")
          ...
          (cond ((eql numargs 2)
                 (let ((type1 (derive-type arg1)))
                   (cond ((and (neq type1 :unknown)
                          ...
                          (format t "p2-delete list-delete-eq~%")
                          (p2-function-call (list 'LIST-DELETE-EQ arg1 arg2) target)
                          t)
                         (t
                          (format t "p2-delete list-delete-eql~%")
  ...
  (when (check-arg-count form 4)
    (let* ((args (%cdr form))
      ...
      (when (and (integer-constant-value type2)
                 ...
                 (flushable arg2)
                 (flushable arg3))
        (format t "p2-%dpb optimized case~%")
...
(defun p2-fill (form target)
  ...
  (let* ((args (cdr form))
         ...
         (arg1 (car args))
         (type1 (derive-type arg1)))
    (format t "p2-fill numargs = ~S derived type is ~S~%" numargs type1)
    (cond ((and (eql numargs 2)
                (neq type1 :unknown)
                (subtypep type1 'SIMPLE-BIT-VECTOR))
           (format t "p2-fill simple-bit-vector-fill case~%")
              ...
              ((and (eql numargs 4)
               ...
               (cond ((zerop *safety*)
                     ...
                     ((and (neq type1 :unknown)
                      ...
                      t)
                     (t
                      (format t "p2-find no optimization~%")
                      nil)))
              (t
               (format t "p2-find no optimization~%")
               nil))))))
...
;;                       (format t "already have constraint for ~S: ~S~%"
;;                               (var-name var) (constraint-type constraint))
;;                       (format t "new constraint: ~S~%" type)
                      (when (subtypep type (constraint-type constraint))
;;                         (format t "adding new constraint~%")
                        (push (make-constraint :var var :type type) *constraints*)))
                     (t
                      (setq type (canonicalize-type type))
;;                       (format t "adding constraint var = ~S type = ~S~%" (var-name var) type)
                      (push (make-constraint :var var :type type) *constraints*))))))))
      ...
      (when (and (eql numargs 1)
        ...
        (let ((type (case operator
          ...
          (cond ((eq type :unknown)
                ...
                (t
                 (setq type (canonicalize-type type))
                 (format t "adding negative constraint var = ~S type = ~S~%" (var-name var) type)
                 (push (make-constraint :var var :type type) *constraints*))))))))
    ...
    (let ((op (%car form))
      ...
      (cond ((zerop *safety*)
            ...
            ((and (neq (setq type (derive-type arg)) :unknown)
             ...
             (process-1-arg arg target t))
            (t
             (format t "%p2-require-type full call to ~S~%" op)
             (process-1-arg arg :default t)
             (emit-call-1 op target)
             (when (var-ref-p arg)
...
;;                (let ((*print-structure* nil)
;;                      (*print-array* nil))
;;                  (format t "p2-typep arg2 = ~S type = ~S~%" (cadr arg2) type))
...
;;                       (let ((*print-structure* nil)
;;                             (*print-array* nil))
;;                         (format t "p2-typep new-form = ~S~%" new-form))
                      (p2-function-call new-form target)
                      t)
                     ((and (consp type)
                           (eq (%car type) 'MEMBER))
...
;;                       (let ((*print-structure* nil)
;;                             (*print-array* nil))
;;                         (format t "p2-typep form = ~S new-form = ~S~%" form new-form))
                      (p2 new-form target)
                      t)
                     (t
;;                       (let ((*print-structure* nil)
;;                             (*print-array* nil))
;;                         (format t "p2-typep full call arg2 = ~S type = ~S~%" arg2 type))
                      nil))))
            (t
;;              (let ((*print-structure* nil)
;;                    (*print-array* nil))
;;                (format t "p2-typep full call arg2 = ~S~%" arg2))
             nil)))))
...
;;                     (format t "broken handler ~S~%" handler)
;;                     (break))
                  (unless handled
                    (p2-function-call form target))
                  )
