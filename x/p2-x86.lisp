...
(defun p2-block (form target)
  (let* ((block (cadr form))
    ...
    (cond ((block-non-local-return-p block)
           (mumble "p2-block non-local return case~%")
...
(defun p2-schar (form target)
  (when (length-eql form 3)
    (let* ((args (%cdr form))
           ...
           type1)
      (cond ((zerop *safety*)
             (mumble "p2-schar safety is zero~%")
             (clear-register-contents)
  ...
  (let* ((arg1 (%car args))
    ...
    (cond ((eq targets :stack)
          ...
          ((and (consp targets)
                ...
                (eq (first targets) :eax)
                (reg32-p (second targets)))
           (let ((reg1 :eax)
                 (reg2 (second targets)))
             (cond ((and (constant-or-local-var-ref-p arg1)
                         (constant-or-local-var-ref-p arg2))
  ...
  (let ((var (var-ref-var form)))
    ...
    (cond ((var-constant-p var)
          ...
          ((var-closure-index var)
           ...
           (emit-move-local-to-register (compiland-closure-data-index *current-compiland*) :ecx)
           (clear-register-contents :ecx)
           (emit-move-relative-to-register :ecx (var-closure-index var) :eax)
           (set-register-contents :eax var)
           (move-result-to-target target)
           (when (reg32-p target)
...
;;                       (mumble "p2-ash unsigned-byte 32 case (optimized)~%")
                      (let ((LABEL1 (make-label))
                        ...
                        (cond ((> (- shift +fixnum-shift+) -32)
                               (mumble "p2-ash unsigned-byte 32 case (optimized) a~%")
                               (inst :shr (- (- shift +fixnum-shift+)) :eax))
                              (t
                               (mumble "p2-ash unsigned-byte 32 case (optimized) b~%")
                               (unbox-fixnum :eax)
                               (inst :shr (- shift) :eax)))
                        (box-fixnum :eax)
                        (label LABEL1)
    ...
    (let ((arg (%cadr form))
      ...
      (cond ((zerop *safety*)
            ...
            ((eq (setq type (derive-type arg)) 'SYMBOL)
             (p2 arg target))
            (t
             (mumble "p2-require-symbol target = ~S~%" target)
             (process-1-arg arg :eax t)
             (let* ((EXIT (make-label))
                    (common-labels (compiland-common-labels *current-compiland*))
