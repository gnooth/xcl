...
bool ReaderError::typep(Value type) const
{
  if (symbolp(type))
    return (type == S_reader_error || type == S_parse_error || type == S_stream_error 
            || type == S_error || type == S_serious_condition || type == S_condition
            || type == S_standard_object || type == S_atom || type == T);
  else
    return (type == C_reader_error || type == C_parse_error || type == C_stream_error 
