...
bool DivisionByZero::typep(Value type) const
{
  if (symbolp(type))
    return (type == S_division_by_zero || type == S_arithmetic_error || type == S_error
            || type == S_serious_condition || type == S_condition
            || type == S_standard_object || type == S_atom || type == T);
  else
    return (type == C_division_by_zero || type == C_arithmetic_error || type == C_error
