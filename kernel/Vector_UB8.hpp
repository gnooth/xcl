...
#ifndef __VECTOR_UB8_HPP
#define __VECTOR_UB8_HPP

class Vector_UB8 : public AbstractVector
{
private:
...
public:
  Vector_UB8(INDEX capacity, Value fill_pointer);

  // displaced
  Vector_UB8(INDEX capacity, AbstractArray * array, INDEX offset, Value fill_pointer);
...
inline bool vector_ub8_p(Value value)
{
  return (typed_object_p(value)
          && the_typed_object(value)->widetag() == WIDETAG_VECTOR_UB8);
}

inline Vector_UB8 * the_vector_ub8(Value value)
{
  assert(vector_ub8_p(value));
  return reinterpret_cast<Vector_UB8 *>(value - LOWTAG_TYPED_OBJECT);
}

inline Vector_UB8 * check_vector_ub8(Value value)
{
  if (vector_ub8_p(value))
    return the_vector_ub8(value);
  Value expected_type = list2(S_vector, UB8_TYPE);
  signal_type_error(value, expected_type);
  // Not reached.
  return NULL;
}

#endif // Vector_UB8.hpp
