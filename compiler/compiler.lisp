...
;;              (aver (symbolp target))
    ...
    (dotimes (i (length code))
      (let ((instruction (svref code i)))
        (declare (type ir2-instruction instruction))
        (when (eq (operator instruction) :label)
          (let ((target (operand1 instruction)))
;;             (aver (not (null target)))
;;             (aver (symbolp target))
...
(defun combine-binary-data ()
  (let ((code (coerce *code* 'simple-vector))
        changed)
    (declare (type simple-vector code))
    (dotimes (i (1- (length code)))
      (let ((instruction-1 (svref code i)))
        (when instruction-1
          (when (eq (instruction-kind instruction-1) :bytes)
            (let ((instruction-2 (svref code (1+ i))))
              (when (eq (instruction-kind instruction-2) :bytes)
;;                 (mumble "combining instructions ~D and ~D~%" i (1+ i))
                (let* ((size (+ (instruction-size instruction-1)
                                (instruction-size instruction-2)))
                       (data (make-array size :element-type '(unsigned-byte 8)))
                       (j 0))
                  (dolist (byte (instruction-data instruction-1))
                    (setf (aref data j) byte)
                    (incf j))
                  (dolist (byte (instruction-data instruction-2))
                    (setf (aref data j) byte)
                    (incf j))
                  (setf (svref code i) (make-instruction :byte-vector size data))
                  (setf (svref code (1+ i)) nil))
;;                 (print-instruction (svref code i))
                (setq changed t)))))))
    (when changed
      (setq *code* (coerce (delete nil code) 'list)))))
