...
(defun derive-type-concatenate (form)
  (when (>= (length form) 3)
    (let* ((type-arg (%cadr form)))
      (when (quoted-form-p type-arg)
        (let ((type (canonicalize-type (%cadr type-arg))))
          (when (subtypep type 'SEQUENCE)
            (return-from derive-type-concatenate type))))))
  :unknown)
...
(defun derive-type-copy-seq (form)
  (when (length-eql form 2)
    (let* ((arg (%cadr form))
           (arg-type (derive-type arg)))
      (cond ((eq arg-type :unknown)
             (return-from derive-type-copy-seq :unknown))
            ((subtypep arg-type 'LIST)
             (return-from derive-type-copy-seq arg-type))
            ((subtypep arg-type 'VECTOR)
             (when (and (consp arg-type)
                        (eq (%car arg-type) 'ARRAY))
               (setf (car arg-type) 'SIMPLE-ARRAY))
             (return-from derive-type-copy-seq arg-type)))))
  :unknown)

(defknown derive-type-subseq (t) t)
(defun derive-type-subseq (form)
  (when (>= (length form) 2)
    (let* ((sequence-arg (%cadr form))
           (sequence-type (derive-type sequence-arg)))
      (cond ((eq sequence-type :unknown)
             (return-from derive-type-subseq :unknown))
            ((subtypep sequence-type 'LIST)
             (return-from derive-type-subseq 'LIST))
            ((subtypep sequence-type 'VECTOR)
             ;; FIXME element type
             (return-from derive-type-subseq (canonicalize-type 'SIMPLE-VECTOR))))))
  :unknown)

(defknown derive-type-delete/remove (t) t)
(defun derive-type-delete/remove (form)
  (declare (type list form))
  (when (>= (length form) 3)
    (let* ((args (cdr form))
           (sequence-arg (%cadr args))
           (sequence-type (derive-type sequence-arg)))
      (cond ((eq sequence-type :unknown)
             (return-from derive-type-delete/remove :unknown))
            ((subtypep sequence-type 'LIST)
             (return-from derive-type-delete/remove 'LIST))
            ((subtypep sequence-type 'VECTOR)
             ;; FIXME element type
             (return-from derive-type-delete/remove (canonicalize-type 'VECTOR))))))
  :unknown)
...
(install-derive-type-handler 'concatenate            'derive-type-concatenate)
(install-derive-type-handler 'copy-seq               'derive-type-copy-seq)
(install-derive-type-handler 'delete                 'derive-type-delete/remove)
(install-derive-type-handler 'delete-if              'derive-type-delete/remove)
(install-derive-type-handler 'delete-if-not          'derive-type-delete/remove)
...
(install-derive-type-handler 'remove                 'derive-type-delete/remove)
(install-derive-type-handler 'remove-if              'derive-type-delete/remove)
(install-derive-type-handler 'remove-if-not          'derive-type-delete/remove)
...
(install-derive-type-handler 'subseq                 'derive-type-subseq)
(install-derive-type-handler 'subseq2                'derive-type-subseq)
(install-derive-type-handler 'subseq3                'derive-type-subseq)
