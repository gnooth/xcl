...
(defun p2-require-symbol (form target)
  (when (check-arg-count form 1)
    (let ((arg (%cadr form)))
      (cond ((or (zerop *safety*) (eq (derive-type arg) 'SYMBOL))
             (p2 arg target))
            (t
             (mumble "p2-require-symbol~%")
             (let* ((EXIT (make-label))
                    (ERROR (common-error-label 'error-not-symbol $ax)))
               (process-1-arg arg $ax t)
               (inst :compare-immediate nil $ax)
               ...
               (when target
                 (move-result-to-target target))
               (when (var-ref-p arg)
                 (add-type-constraint (var-ref-var arg) 'SYMBOL))))))
    t))
