    ...
    (case numargs
      (1 (let* ((arg (%car args))
           ...
           (cond ((eq type :unknown)
                  (mumble "p2-floor emitting call to floor-1 (unknown argument type)~%")
                 ...
                 (t
                  (mumble "p2-floor emitting call to floor-1 (unsafe case)~%")
                  (p2-function-call-1 'floor-1 args target)))
           t))
      (2 (let* ((arg1 (%car args))
                (arg2 (%cadr args))
                (type1 (derive-type arg1))
                (type2 (derive-type arg2)))
;;            (mumble "p2-floor type1 = ~S type2 = ~S~%" type1 type2)
           (cond ((or (eq type1 :unknown)
                 ...
                 ((and (integer-type-p type1)
                       (fixnump arg2)
                       (member arg2 '(2 4 8 16 32 64 128 256)))
                  (let* ((shift (1- (integer-length arg2)))
                         (mask (1- arg2)))
                    (aver (eql arg2 (ash 1 shift)))
                    (mumble "p2-floor optimized case shift = ~S mask = ~S~%" shift mask)
                    (let ((thread-reg (compiland-thread-register *current-compiland*)))
                      (aver thread-reg)
                      (process-1-arg arg1 :rax t)
                      (unbox-fixnum :rax)
                      (inst :push :rax)
                      (inst :shr shift :rax) ; unboxed quotient in rax
                      (inst :shl +fixnum-shift+ :rax)
                      (inst :pop :rdx)
                      (inst :and mask :rdx) ; remainder in rdx
                      (inst :shl +fixnum-shift+ :rdx)
