...
(defun unbox-character (register)
  (inst :sar +character-shift+ register)
  (clear-register-contents register))
...
(defun emit-byte (byte)
  (emit (make-ir2-instruction :bytes (list byte) nil)))
          ...
          (unless (eq type1 :unknown)
            (cond ((subtypep type1 output-type)
                  ...
                  ((and (eq output-type 'SIMPLE-STRING)
                   ...
                   (emit-call-1 'copy-string target)
                   t)
                  ((and (eq output-type 'LIST)
                        (subtypep type1 'VECTOR))
                   (p2 (list 'COERCE-VECTOR-TO-LIST arg1) target)
                   t)
                  ((and (memq output-type '(VECTOR SIMPLE-VECTOR))
                        (subtypep type1 'LIST))
                   (p2 (list 'COERCE-LIST-TO-SIMPLE-VECTOR arg1) target)
                   t)
                  ((and (subtypep output-type 'VECTOR)
                        (subtypep type1 'LIST))
                   (p2 (list 'COERCE-LIST-TO-VECTOR arg1 arg2) target)
                   t)
                  ((eq output-type 'FUNCTION)
                   (p2 (list 'COERCE-TO-FUNCTION arg1) target)
                   t))))))))

(defun p2-copy-seq (form target)
  (when (length-eql form 2)
    (let* ((arg (%cadr form))
           (type (derive-type arg)))
      (mumble "p2-copy-seq type = ~S~%" type)
      (unless (eq type :unknown)
        (cond ((eq type 'LIST)
               (p2 `(copy-list ,arg) target)
               t)
              ((subtypep type 'SIMPLE-BIT-VECTOR)
               (p2 `(copy-simple-bit-vector ,arg) target)
               t)
              ((subtypep type 'STRING)
               (p2 `(copy-string ,arg) target)
               t)
              ((subtypep type 'VECTOR)
               (p2 `(copy-vector ,arg) target)
               t))))))
...
;;     (mumble "p2-fill numargs = ~S derived type is ~S~%" numargs type1)
    (cond ((and (eql numargs 2)
                (neq type1 :unknown))
           (cond ((subtypep type1 'SIMPLE-BIT-VECTOR)
    ...
    (case numargs
      (1 (let* ((arg (%car args))
           ...
           (cond ((eq type :unknown)
                  (mumble "p2-floor emitting full call to floor-1 (unknown argument type)~%")
                  (p2-function-call-1 'floor-1 args target))
                 ((subtypep type 'REAL)
                  (process-1-arg arg :default t)
                  (mumble "p2-floor emitting call to floor-1 (safe case)~%")
                  (emit-call-1 'floor-1 target))
                 (t
                  (mumble "p2-floor emitting full call to floor-1 (unsafe case)~%")
                  (p2-function-call-1 'floor-1 args target)))
           t))
      (2 (let* ((arg1 (%car args))
                (arg2 (%cadr args))
                (type1 (derive-type arg1))
                (type2 (derive-type arg2)))
           (cond ((or (eq type1 :unknown)
                      (eq type2 :unknown))
                  (mumble "p2-floor emitting full call to floor-2 (unknown argument type)~%")
                  (p2-function-call-2 'floor-2 args target))
                 ;; REVIEW
                 ((and (fixnum-type-p type1)
                       (fixnump arg2)
                       (member arg2 '(2 4 8 16 32 64)))
                  (let* ((shift (1- (integer-length arg2)))
                         (mask (1- arg2)))
                    (mumble "p2-floor optimized case arg2 = ~S shift = ~S mask = ~S~%" arg2 shift mask)
                    ...
                    (let ((reg1 #+x86-64 :rax #+x86 :eax)
                      ...
                      (unbox-fixnum reg1)
                      (inst :push reg1)
                      (inst :shr shift reg1) ; unboxed quotient in reg1
                      (box-fixnum reg1)
                      (inst :pop reg2)
                 ...
                 ((and (subtypep type1 'REAL)
                       ...
                       (not (zerop arg2)))
                  (process-2-args args :default t)
                  (mumble "p2-floor emitting call to floor-2 (safe case)~%")
                  (emit-call-2 'floor-2 target))
                 (t
                  (mumble "p2-floor emitting call to floor-2 (unsafe case)~%")
                  (p2-function-call-2 'floor-2 args target)))
           t))
      (t nil))))

(defknown p2-ceiling (t t) t)
(defun p2-ceiling (form target)
  (let* ((args (cdr form))
         (numargs (length args)))
    (case numargs
      (1 (let* ((arg (%car args))
           ...
           (cond ((and (neq type :unknown)
                       (subtypep type 'REAL))
                  (process-1-arg arg :default t)
                  (mumble "p2-ceiling emitting call to ceiling-1 (safe case)~%")
                  (emit-call-1 'ceiling-1 target))
                 (t
                  (mumble "p2-ceiling emitting full call to ceiling-1 (unsafe case)~%")
                  (p2-function-call-1 'ceiling-1 args target)))
           t))
      (2 (let* ((arg1 (%car args))
                (arg2 (%cadr args))
                (type1 (derive-type arg1))
           ...
           (cond ((or (eq type1 :unknown)
                      (eq type2 :unknown))
                  (mumble "p2-ceiling emitting full call to ceiling-2 (unknown argument type)~%")
                  (p2-function-call-2 'ceiling-2 args target))
                 ((and (subtypep type1 'REAL)
                       (realp arg2)
                       (not (zerop arg2)))
                  (process-2-args args :default t)
                  (mumble "p2-ceiling emitting call to ceiling-2 (safe case)~%")
                  (emit-call-2 'ceiling-2 target))
                 (t
                  (mumble "p2-ceiling emitting full call to ceiling-2 (unsafe case)~%")
                  (p2-function-call-2 'ceiling-2 args target)))
           t))
      (t nil))))

(defknown p2-truncate (t t) t)
(defun p2-truncate (form target)
  (let* ((args (cdr form))
         (numargs (length args)))
    (case numargs
      (1 (let* ((arg (%car args))
           ...
           (cond ((and (neq type :unknown)
                       (subtypep type 'REAL))
                  (process-1-arg arg :default t)
                  (mumble "p2-truncate emitting call to truncate-1 (safe case)~%")
                  (emit-call-1 'truncate-1 target))
                 (t
                  (mumble "p2-truncate emitting full call to truncate-1 (unsafe case)~%")
                  (p2-function-call-1 'truncate-1 args target)))
           t))
      (2 (let* ((arg1 (%car args))
                (arg2 (%cadr args))
                (type1 (derive-type arg1))
           ...
           (cond ((or (eq type1 :unknown)
                      (eq type2 :unknown))
                  (mumble "p2-truncate emitting full call to truncate-2 (unknown argument type)~%")
                  (p2-function-call-2 'truncate-2 args target))
                 ((and (subtypep type1 'REAL)
                       (realp arg2)
                       (not (zerop arg2)))
                  (process-2-args args :default t)
                  (mumble "p2-truncate emitting call to truncate-2 (safe case)~%")
                  (emit-call-2 'truncate-2 target))
                 (t
                  (mumble "p2-truncate emitting full call to truncate-2 (unsafe case)~%")
                  (p2-function-call-2 'truncate-2 args target)))
           t))
      (t nil))))

(defknown p2-rem (t t) t)
(defun p2-rem (form target)
  (when (check-arg-count form 2)
    (let* ((args (%cdr form))
           (arg1 (%car args))
           (arg2 (%cadr args))
...
(install-p2-test-handler 'functionp             'p2-test-functionp)
(install-p2-test-handler 'listp                 'p2-test-listp)
(install-p2-test-handler 'neq                   'p2-test-neq)
(install-p2-test-handler 'numberp               'p2-test-numberp)
(install-p2-test-handler 'plusp                 'p2-test-plusp)
(install-p2-test-handler 'simple-vector-p       'p2-test-simple-vector-p)
          ...
          (dolist (decl decls)
            (let ((declaration-identifier (car decl)))
              (case declaration-identifier
                ((DYNAMIC-EXTENT FTYPE INLINE NOTINLINE OPTIMIZE IGNORE IGNORABLE SPECIAL)
                 ;; nothing to do here
                 )
                (TYPE
                 (dolist (name (cddr decl))
                   (let ((var (find-var name *visible-variables*)))
                     (when var
                       (push (copy-structure var) saved-vars)
                       (setf (var-declared-type var) (cadr decl))
                       (setf (var-type-check-form var) nil)
                       (p1-check-var-type var)
                       (push var vars-to-be-checked)))))
                (t
                   ...
                   (dolist (name (cdr decl))
                     (let ((var (find-var name *visible-variables*)))
                       (when var
                         ...
                         (p1-check-var-type var)
                         (push var vars-to-be-checked)))))))))))
      (dolist (var (nreverse vars-to-be-checked))
        (p2-check-var-type var nil))
      (p2-progn-body body target)
      (dolist (saved-var saved-vars)
        (let ((var (find-var (var-name saved-var) *visible-variables*)))
          (aver (not (null var)))
          (when var
            (setf (var-declared-type var) (var-declared-type saved-var))
            (setf (var-derived-type var) (var-derived-type saved-var))
            (setf (var-type-check-form var) (var-type-check-form saved-var))))))))
...
;;                  (mumble "p2-typep arg2 = ~S type = ~S~%" (cadr arg2) type))
               (cond ((subtypep derived-type type)
                      (mumble "p2-typep skipping call~%")
                      (p2 arg1 nil) ; for effect)
                      (p2 t target)
                      t)
                     ((eq type 'structure-object)
...
;;           (mumble "considering var ~S~%" (var-name var))
          (setf (var-register-worthiness var)
                (+ (var-register-worthiness var)
                   (var-reads var)
                   (var-writes var)))
;;           (mumble "worthiness = ~S~%" (var-register-worthiness var))
          (when (>= (var-register-worthiness var) threshold)
            (unless (or (var-special-p var)
                        (var-used-non-locally-p var))
;;               (mumble "it's a keeper!~%")
              (push var candidates))))
        (when candidates
          (when (> (length candidates) (length *available-registers*))
;;             (mumble "calling sort~%")
            (setq candidates (sort candidates #'> :key #'var-register-worthiness)))
;;             (dolist (var locals)
;;               (mumble "~S ~S~%" (var-name var) (var-register-worthiness var)))
          (dolist (var candidates)
            (declare (type var var))
            (aver (not (var-special-p var)))
            (aver (not (var-used-non-locally-p var)))
            ;;           (unless (or (var-special-p var)
            ;;                       (var-used-non-locally-p var))
...
;;                        (mumble "assign-registers-for-locals var = ~S reg = ~S worthiness = ~S~%"
;;                                (var-name var) reg (var-register-worthiness var))
