...
(defun p2-require-widetag-bit (register widetag-bit error-function)
  (aver (eq register :eax))
  (let ((ERROR (common-label *current-compiland* error-function :eax)))
    (inst :mov :al :dl)
    (clear-register-contents :edx)
    (inst :and +lowtag-mask+ :dl)
    ...
    (inst :testl widetag-bit `(,(- +widetag-offset+ +typed-object-lowtag+) :eax))
...
(defun p2-require-widetag (register widetag error-function)
  ...
  (let ((ERROR (common-label *current-compiland* error-function register)))
    ...
    (inst :cmpl widetag `(,(- +widetag-offset+ +typed-object-lowtag+) :eax))
    (emit-jmp-short :nz ERROR)))
