...
(defun p2-set-sbit1 (form target)
  (when (length-eql form 4)
    (let* ((op (%car form))
      ...
      (cond ((or (zerop *safety*)
                 (eq op '%set-sbit1))
             (mumble "p2-set-sbit1 optimized case~%")
             (let ((LABEL1 (make-label))
                   (LABEL2 (make-label)))
               (process-3-args args '(:edx :ecx :eax) t)
               (unbox-fixnum :ecx) ; unboxed index in rsi
               (clear-register-contents :ecx)
               (inst :test :al :al)
               (emit-jmp-short :z LABEL1)
               (inst :bts :ecx `(,(- +simple-bit-vector-data-offset+ +typed-object-lowtag+) :edx))
               (emit-jmp-short t LABEL2)
               (label LABEL1)
               (inst :btr :ecx `(,(- +simple-bit-vector-data-offset+ +typed-object-lowtag+) :edx))
               (label LABEL2)
               (move-result-to-target target)))
            (t
...
(defun call-with-vectorized-args (op args)
  (let ((numargs (length args))
    ...
    (inst :push numargs)
    (emit-call op)
    (inst :add (* (+ numargs 2) +bytes-per-word+) :esp)))
             ...
             (cond ((and (fixnum-type-p type1)
                   ...
                   (t
                    (let ((FULL-CALL (make-label))
                          (EXIT (make-label)))
                      (process-1-arg arg1 :eax t)
                      (inst :mov :eax :ecx) ; save boxed arg for full call
                      (unless (fixnum-type-p type1)
                        (inst :test +fixnum-tag-mask+ :al)
                        (emit-jmp-short :nz FULL-CALL))
                      (unbox-fixnum :eax)
                      (inst :cmp most-negative-fixnum :eax)
      ...
      (cond ((zerop *safety*)
            ...
            (t
             ...
             (let ((FAIL (make-label)))
               ...
               (inst :testl +widetag-vector-bit+ `(,(- +widetag-offset+ +typed-object-lowtag+) :eax))
               ...
               (let ((*current-segment* :elsewhere))
                 (label FAIL)
                 (inst :push :eax)
                 (emit-call-1 'error-not-vector nil)
                 (inst :exit) ; FIXME
                 (when (var-ref-p arg)
      ...
      (cond ((zerop *safety*)
            ...
            (t
             ...
             (let* ((common-labels (compiland-common-labels *current-compiland*))
               ...
               (inst :cmpl +simple-vector-widetag+ `(,(- +widetag-offset+ +typed-object-lowtag+) :eax))
               (emit-jmp-short :ne REQUIRE-SIMPLE-VECTOR-ERROR)
               (when target
                 (move-result-to-target target))
               (when (var-ref-p arg)
      ...
      (2
       (let ((arg1 (%car args))
         ...
         (cond ((and (eq arg1 arg2)
                     (constantp arg1))
                (p2-constant arg1 :eax)
                (inst :mov thread-var :ecx)
                (clear-register-contents :ecx)
                (inst :mov :eax `(,+values-offset+ :ecx))
                (inst :mov :eax `(,(+ +values-offset+ +bytes-per-word+) :ecx)))
               (t
