...
;;              (setq locals-allocated-p t)
            ...
            (:allocate-thread-var
;;              (aver (not ":allocate-thread-var case needs code~%"))
...
;;                (aver (null (var-index var)))
            ...
            (:allocate-locals
;;              (aver (not ":allocate-locals case needs code~%"))
             (aver (not locals-allocated-p))
             (unless (compiland-omit-frame-pointer compiland)
               (let ((locals (reverse (compiland-local-vars compiland))))
                 (when locals
                   (dolist (var locals)
                     (declare (type var var))
                     (unless (or (var-special-p var) (var-register var))
