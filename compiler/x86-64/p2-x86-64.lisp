...
(defun p2-require-widetag-bit (reg mask common-label-function)
  (aver (eq reg :rax))
  (let ((ERROR (funcall common-label-function *current-compiland* :rax)))
    (inst :mov :al :dl)
    (clear-register-contents :rdx)
    (inst :and +lowtag-mask+ :dl)
    (inst :cmp +typed-object-lowtag+ :dl)
    (emit-jmp-short :ne ERROR)
    (aver (typep mask '(unsigned-byte 32)))
    (inst :testq mask `(,(- +widetag-offset+ +typed-object-lowtag+) :rax))
    (emit-jmp-short :z ERROR)))

(defun p2-require-widetag (reg widetag common-label-function)
  (aver (eq reg :rax))
  (let ((ERROR (funcall common-label-function *current-compiland* :rax)))
    (inst :mov :al :dl)
    (clear-register-contents :rdx)
...
(defun p2-require-simple-bit-vector (form target)
  (let* ((op (%car form))
         (arg (%cadr form))
         (type (derive-type arg)))
    (aver (eq op 'require-simple-bit-vector))
    (cond ((zerop *safety*)
           (p2 arg target))
          ...
          (t
           ...
           ;;              (inst :testq +widetag-vector-bit+ `(,(- +widetag-offset+ +typed-object-lowtag+) :rax))
           ;;              (emit-jmp-short :z ERROR-NOT-VECTOR))
           (p2-require-widetag :rax +simple-bit-vector-widetag+ 'common-label-error-not-simple-bit-vector)
           (when target
             (move-result-to-target target))
           (when (var-ref-p arg)
...
;;                (emit-jmp-short :ne REQUIRE-SIMPLE-STRING-ERROR))
             (p2-require-widetag :rax +simple-string-widetag+ 'common-label-error-not-simple-string)
             (when target
;;                (inst :mov :rdi :rax)
               (move-result-to-target target))
             (when (var-ref-p arg)
