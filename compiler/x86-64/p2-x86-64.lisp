...
;;     (inst :sub (* (+ numreq numopt) +bytes-per-word+) :rsp)
    (inst :mov '(:rsi) :rax)
;;     (inst :mov :rax '(:rsp))
    (inst :push :rax)
    (setf (var-index (find-var (first lambda-list) (compiland-arg-vars compiland))) 0)
    (inst :cmp 2 :rdi)
    (emit-jmp-short :nz NOT-SUPPLIED)
    (inst :mov `(,+bytes-per-word+ :rsi) :rax)
;;     (inst :mov :rax `(,+bytes-per-word+ :rsp))
    (inst :push :rax)
    (emit-jmp-short t EXIT)
    (label NOT-SUPPLIED)
    (p2-constant nil :stack)
    (label EXIT)
    (setf (var-index (find-var (third lambda-list) (compiland-arg-vars compiland))) 1)
;;     (trivial-allocate-locals compiland)
    (allocate-locals compiland 2)
    (inst :align-stack)
    (inst :initialize-thread-register)
    (let ((*current-segment* :elsewhere))
      (label ERROR)
      ...
      (inst :mov (fixnumize 1) :rdx)
      (inst :mov (fixnumize 2) :rcx)
      (emit-call 'error-wrong-number-of-arguments)
      (emit-exit))))
