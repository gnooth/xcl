...
(defmacro define-common-label-for-error (error-function)
  (let ((name (intern (concatenate 'string "COMMON-LABEL-" (symbol-name error-function))))
        (prefix (concatenate 'string (symbol-name error-function) "-")))
  `(progn
     (defknown ,name (t t) t)
...
;; (define-common-label-for-error common-label-error-not-simple-string
;;   "ERROR-NOT-SIMPLE-STRING-" error-not-simple-string)
(define-common-label-for-error error-not-cons)
(define-common-label-for-error error-not-list)
(define-common-label-for-error error-not-vector)
...
(defun p2-let/let* (form target)
  ...
  (let* ((*visible-variables* *visible-variables*)
         ...
         (last-special-binding-var nil)
         (thread-register nil))
    (declare (type cblock block))
    (when (setq last-special-binding-var (block-last-special-binding-var block))
      (setq thread-register (compiland-thread-register *current-compiland*))
      (cond (thread-register
             (inst :mov `(,+last-special-binding-offset+ ,thread-register) :rax)
             (clear-register-contents :rax))
            (t
      ...
      (cond ((and values-length
            ...
            (t
             ...
             (let ((*current-segment* :elsewhere))
               ...
               (inst :pop :rax)
               (emit-jmp-short t LABEL2))))
      ;; primary value is in rax
      ;; pointer to values vector is in rsi
      (label LABEL2)
      (let ((index 0))
        (dolist (var vars)
          (declare (type var var))
          (unless (eql index 0)
            (inst :mov `(,index ,:rsi) :rax)
            (clear-register-contents :rax))
          (cond ((var-special-p var)
                 (inst :push :rsi)
                 (inst :mov :rax :rdx)
...
(defknown p2-set-sbit1 (t t) t)
(defun p2-set-sbit1 (form target)
  (when (length-eql form 4)
    (let* ((op (%car form))
           (args (%cdr form)))
      (cond ((or (zerop *safety*)
             ...
             (let ((LABEL1 (make-label))
                   (LABEL2 (make-label)))
               (process-3-args args '(:rdi :rsi :rax) t)
               (unbox-fixnum :rsi) ; unboxed index in rsi
               (clear-register-contents :rsi)
               (inst :test :al :al)
               (emit-jmp-short :z LABEL1)
               (inst :bts :rsi `(,(- +simple-bit-vector-data-offset+ +typed-object-lowtag+) :rdi))
               (emit-jmp-short t LABEL2)
               (label LABEL1)
               (inst :btr :rsi `(,(- +simple-bit-vector-data-offset+ +typed-object-lowtag+) :rdi))
               (label LABEL2)
               (move-result-to-target target)))
            (t
             (process-3-args args :default t)
             (emit-call-3 'set-sbit1 target))))
    t))
...
(defknown vectorize-args (list) t)
(defun vectorize-args (args)
...
(defun p2-plusp (form target)
  (when (length-eql form 2)
    (let* (;(operator (%car form))
           (arg (%cadr form))
           (type (derive-type arg)))
      (cond ((subtypep type '(INTEGER 0 1))
             (mumble "p2-plusp bit case~%")
             (let ((NO (make-label))
                   (EXIT (make-label)))
               (process-1-arg arg :rax t)
               (inst :test :al :al)
               (emit-jmp-short :z NO)
               (p2 t :rax)
               (inst :jmp-short t EXIT)
               (label NO)
               (p2 nil :rax)
               (label EXIT)
               (move-result-to-target target)))
            (t
             (mumble "p2-plusp default case type = ~S~%" type)
             (let ((FULL-CALL (make-label))
                   (NO (make-label))
                   (EXIT (make-label)))
               (process-1-arg arg :rax t)
...
(defun p2-minusp (form target)
  (when (length-eql form 2)
    (let* ((arg (%cadr form))
           (type (derive-type arg)))
      (mumble "p2-minusp type = ~S~%" type)
      (process-1-arg arg :default t)
      (emit-call-1 'minusp target))
    t))
...
(defun p2-require-widetag-bit (register widetag-bit error-function)
  (aver (eq register :rax))
;;   (let ((ERROR (funcall common-label-error-function *current-compiland* :rax)))
  (let ((ERROR (common-label *current-compiland* error-function :rax)))
    (inst :mov :al :dl)
    (clear-register-contents :rdx)
    (inst :and +lowtag-mask+ :dl)
    ...
    (inst :testq widetag-bit `(,(- +widetag-offset+ +typed-object-lowtag+) :rax))
...
;;   (let ((ERROR (funcall common-label-error-function *current-compiland* :rax)))
  (let ((ERROR (common-label *current-compiland* error-function register)))
    ...
    (inst :cmpq widetag `(,(- +widetag-offset+ +typed-object-lowtag+) :rax))
    (emit-jmp-short :nz ERROR)))
  ...
  (let* ((op (%car form))
    ...
    (cond ((zerop *safety*)
          ...
          (t
           ...
           (process-1-arg arg :rax t)
;;            (let ((ERROR-NOT-VECTOR (common-label-error-not-vector *current-compiland* :rax)))
;;              (inst :mov :al :dl)
;;              (clear-register-contents :rdx)
;;              (inst :and +lowtag-mask+ :dl)
;;              (inst :cmp +typed-object-lowtag+ :dl)
...
;;              (inst :testq +widetag-vector-bit+ `(,(- +widetag-offset+ +typed-object-lowtag+) :rax))
;;              (emit-jmp-short :z ERROR-NOT-VECTOR))

;;            (p2-require-widetag-bit :rax +widetag-vector-bit+ 'common-label-error-not-vector)
           (p2-require-widetag-bit :rax +widetag-vector-bit+ 'error-not-vector)
         ...
         (let* ((arg (%cadr form)))
           (cond ((zerop *safety*)
                 ...
                 (t
                  (mumble "~A~%" ',name)
                  (process-1-arg arg :rax t)
;;                   (p2-require-widetag :rax ,widetag ',error-function)
                  (,require-widetag-function :rax ,widetag-arg ',error-function)
                  (when target
                    (move-result-to-target target))
                  (when (var-ref-p arg)
...
#+nil
(defun p2-require-simple-bit-vector (form target)
  (when (length-eql form 2)
    (let* ((arg (%cadr form)))
      (cond ((zerop *safety*)
             (p2 arg target))
            ((subtypep (derive-type arg) 'SIMPLE-BIT-VECTOR)
             (p2 arg target))
            (t
             (mumble "p2-require-simple-bit-vector~%")
             (process-1-arg arg :rax t)
             (p2-require-widetag :rax +simple-bit-vector-widetag+ 'error-not-simple-bit-vector)
             (when target
               (move-result-to-target target))
             (when (var-ref-p arg)
               (add-type-constraint (var-ref-var arg) 'SIMPLE-BIT-VECTOR)))))
    t))
...
(defun p2-require-simple-string (form target)
  (when (length-eql form 2)
    (let* ((arg (%cadr form)))
      (cond ((zerop *safety*)
             (p2 arg target))
            ((subtypep (derive-type arg) 'SIMPLE-STRING)
             (p2 arg target))
            (t
             (mumble "p2-require-simple-string~%")
             (process-1-arg arg :rax t)
             (p2-require-widetag :rax +simple-string-widetag+ 'error-not-simple-string)
             (when target
               (move-result-to-target target))
             (when (var-ref-p arg)
...
(defun p2-require-simple-vector (form target)
  (when (length-eql form 2)
    (let ((arg (%cadr form)))
      (cond ((zerop *safety*)
             (p2 arg target))
            ((subtypep (derive-type arg) 'SIMPLE-VECTOR)
             (p2 arg target))
            (t
             (process-1-arg arg :rax t)
             (p2-require-widetag :rax +simple-vector-widetag+ 'error-not-simple-vector)
             (when target
               (move-result-to-target target))
             (when (var-ref-p arg)
