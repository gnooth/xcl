...
(defun p2-set-sbit1 (form target)
  (when (length-eql form 4)
    (let* ((op (%car form))
           ...
           (vector-arg (%car args))
           (index-arg (%cadr args))
;;            (value-arg (%caddr args))
           (vector-type (derive-type vector-arg))
           (index-type (derive-type index-arg)))
      (mumble "p2-set-sbit1 vector-type = ~S index-type = ~S~%" vector-type index-type)
      (process-3-args args :default t)
      (cond ((or (zerop *safety*)
                 (eq op '%set-sbit1))
             (mumble "p2-set-sbit1 %set-sbit1 case~%")
             (emit-call-3 '%set-sbit1 target))
            (t
             (mumble "p2-set-sbit1 default case~%")
             (emit-call-3 'set-sbit1 target)))
      nil)))
